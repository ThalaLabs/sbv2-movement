...
// ==============================================================================
const parsedYaml = YAML.parse(fs.readFileSync("./.movement/config.yaml", "utf8"));
...
const { switchboardAddress, guardianQueue } = await client.fetchState();
...
const aptosGuardianQueue = guardianQueue;
const queue = new Queue(client, aptosGuardianQueue);
...
for (const oracle of solanaOracles) {
  if (allOracles.find((o) => o.oracleKey === oracle.pubkey.toBase58())) {
    ...
    const tx = await Oracle.initTx(client, signer, {
      guardianQueue: aptosGuardianQueue,
