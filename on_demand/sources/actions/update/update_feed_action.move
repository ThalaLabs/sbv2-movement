module on_demand::update_action {
    use std::vector;
    use aptos_framework::object;
    use on_demand::aggregator::{Self, Aggregator};
    use on_demand::queue::{Self, Queue};
    use on_demand::oracle::Oracle;
    use on_demand::serialization;
    use on_demand::aggregator_submit_result_action;
    use on_demand::oracle_attest_action;
    use on_demand::decimal;
    ...
    fun actuate<CoinType>(
        ...
        while (idx < vector::length(&update_data)) {
            ...
            if (discriminator == 1) {
                ...
                // if we are able to access historical blockhashes on-chain, or some other on_demand epoch mechanism
     ...
     *  - Removes the on_demand updates and runs them
     *  - Remaining vector will be a vector of non-on_demand byte vectors
     */
    public fun extract_and_run<CoinType>(account: &signer, update_data: &mut vector<vector<u8>>) {
        let idxs_to_remove = vector::empty<u64>();
        let on_demand_updates = vector::empty<vector<u8>>();
        let idx = 0;
        ...
        while (idx < vector::length(update_data)) {
            ...
            if (discriminator == 1 || discriminator == 2) {
                vector::push_back(&mut on_demand_updates, *bytes);
        ...
        // run the updates
        if (vector::length(&on_demand_updates) > 0) {
            actuate<CoinType>(account, on_demand_updates);
        };
    }
